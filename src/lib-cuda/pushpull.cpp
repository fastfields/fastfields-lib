...
// SAME (*batch, *spatial) index, so their spatial extents must match (the per-
// tensor decode relies on this equality). Checks dims [NB, NB+ND).
...
// TNY_MAX_RANK; a larger rank would assert-abort. Reject it with a clear error.
// The Makefile bumps TNY_MAX_RANK to 64 (DLPack max).
...
// spans fit) ON THE HOST, cast the data pointers, and call the teeny device
// launcher (which wraps them as gpu_view anyrank carriers, launches the grid-
// stride kernel on `stream`, and synchronises). Order O and boundary B are
// compile-time; `bnd` (the runtime bound) is forwarded as the driver's `rt` arg
// and consumed only when B == bound_t::Dynamic (the routed bounds). `n1 ==
// nbatch + ndim + 1`; grad's output carries one extra axis.
...
// reg_field::_reg_stream in the reg_field cuda-lib layer.
static inline cudaStream_t _pp_stream(int stream)
{
          ...
          cudaStream_t stream)
{
    ...
    pushpull::pull<D, O, B, reduce_t, scalar_t, offset_t>(
        ...
        static_cast<const scalar_t *>(grid), _zo, _zi, _zg, _to, _ti, _tg, stream);
...
// offset(i32/i64). The bound split mirrors the cpu-lib dispatch: DFT/DCT2/DST2/
// Zero/NoCheck are compiled statically; DCT1/DST1/Replicate route to the single
// Dynamic instantiation (the runtime `bnd` selects inside the kernel). Test builds
// trim order x bound with -DFF_TEST_SPARSE.
...
)
{
    ...
    DISPATCH_PP(_count,
        ...
        out.strides, grid.strides, cstream)
}
