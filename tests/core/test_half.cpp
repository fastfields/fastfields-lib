...
{ oracle_h h = static_cast<oracle_h>(f); uint16_t u; memcpy(&u, &h, 2); return u; }
static float oracle_f16_to_f32(uint16_t u)
{ oracle_h h; memcpy(&h, &u, 2); return static_cast<float>(h); }
static uint16_t oracle_f32_to_bf16(float f)
{ oracle_b b = static_cast<oracle_b>(f); uint16_t u; memcpy(&u, &b, 2); return u; }
static float oracle_bf16_to_f32(uint16_t u)
{ oracle_b b; memcpy(&b, &u, 2); return static_cast<float>(b); }
...
static float    of_h(uint16_t u){ half h;     h.bits = u; return static_cast<float>(h); }
static float    of_b(uint16_t u){ bfloat16 b; b.bits = u; return static_cast<float>(b); }

static bool h_is_nan(uint16_t u) { return ((u >> 10) & 0x1f) == 0x1f && (u & 0x3ff); }
static bool b_is_nan(uint16_t u) { return ((u >>  7) & 0xff) == 0xff && (u & 0x7f);  }
...
int main(int argc, char ** argv)
{
    ...
    if (argc > 1) {
        ...
        if (s <= 0 || !(s & 1)) { std::printf("stride must be a positive odd number\n"); return 2; }
    ...
    static_assert(std::is_trivially_copyable<bfloat16>::value, "bfloat16 memcpy-able");
    ...
    static_assert(std::is_same<ff::compute_type<half>::type, float>::value,     "half computes in float");
    static_assert(std::is_same<ff::compute_type<bfloat16>::type, float>::value, "bf16 computes in float");
    static_assert(std::is_same<ff::compute_type<double>::type, double>::value,  "double computes in double");

    std::printf("core/half.h -- conversion check vs native _Float16 / __bf16\n");
    ...
    // ---- 1. half -> float, all 65536 patterns -----------------------------
    {
        ...
        for (uint32_t u = 0; u < 0x10000u; ++u) {
            ...
            if (h_is_nan(h)) { if (!std::isnan(got)) fail("f16->f32 nan", h, 0, 0); continue; }
            if (bits_of(got) != bits_of(want)) fail("f16->f32", h, bits_of(got), bits_of(want));
        }
    ...
    // ---- 2. bfloat16 -> float, all 65536 patterns -------------------------
    {
        ...
        for (uint32_t u = 0; u < 0x10000u; ++u) {
            ...
            if (b_is_nan(b)) { if (!std::isnan(got)) fail("bf16->f32 nan", b, 0, 0); continue; }
            if (bits_of(got) != bits_of(want)) fail("bf16->f32", b, bits_of(got), bits_of(want));
        }
    ...
    {
        int local = failures; uint64_t n = 0;
        for (uint64_t k = 0; k < 0x100000000ull; k += stride) {
            uint32_t u = static_cast<uint32_t>(k);
            float f = float_of(u); ++n;
            uint16_t got = to_h(f), want = oracle_f32_to_f16(f);
            if (std::isnan(f)) { if (!h_is_nan(got)) fail("f32->f16 nan", u, got, 0); }
            else if (got != want) fail("f32->f16", u, got, want);
        }
        std::printf("  [%s] f32  -> f16   %llu/4294967296 bit patterns (stride %u)\n",
    ...
    // ---- 4. float -> bfloat16, all 2^32 patterns --------------------------
    {
        ...
        for (uint64_t k = 0; k < 0x100000000ull; k += stride) {
            ...
            if (std::isnan(f)) { if (!b_is_nan(got)) fail("f32->bf16 nan", u, got, 0); }
            else if (got != want) fail("f32->bf16", u, got, want);
        }
        std::printf("  [%s] f32  -> bf16  %llu/4294967296 bit patterns (stride %u)\n",
                    failures == local ? "ok" : "!!", (unsigned long long)n, stride);
    }
    ...
    // stated, and they fail readably.
    {
        ...
        check(static_cast<float>(half(-0.25f))     == -0.25f, "half -0.25 round-trip");
        check(static_cast<float>(bfloat16(1.5f))   ==  1.5f,  "bf16 1.5 round-trip");
        check(static_cast<float>(bfloat16(-256.f)) == -256.f, "bf16 -256 round-trip");
        ...
        check(to_h(5.9604644775390625e-08f) == 0x0001, "half smallest subnormal 2^-24");
        check(to_h(std::ldexp(1.0f, -25))   == 0x0000, "half 2^-25 ties to even -> 0");
        check(to_h(std::ldexp(1.5f, -25))   == 0x0001, "half 1.5*2^-25 rounds up");
        check(to_h(std::ldexp(3.0f, -25))   == 0x0002, "half 3*2^-25 ties to even -> 2");
        check(of_h(0x0001) == std::ldexp(1.0f, -24),    "half subnormal decode 2^-24");
        check(of_h(0x03ff) == std::ldexp(1023.0f, -24), "half largest subnormal decode");
        ...
        check(to_h(1.0f + std::ldexp(1.0f, -11)) == 0x3c00, "half tie 1+2^-11 -> even (down)");
        // 1 + 3*2^-11 sits halfway between 0x3c01 and 0x3c02 -> up, to even.
        check(to_h(1.0f + std::ldexp(3.0f, -11)) == 0x3c02, "half tie 1+3*2^-11 -> even (up)");
        // same for bf16: 1 + 2^-8 sits between 0x3f80 and 0x3f81.
        check(to_b(1.0f + std::ldexp(1.0f, -8))  == 0x3f80, "bf16 tie 1+2^-8 -> even (down)");
        check(to_b(1.0f + std::ldexp(3.0f, -8))  == 0x3f82, "bf16 tie 1+3*2^-8 -> even (up)");
        ...
        check(std::isnan(of_h(0x7e00)) && std::isnan(of_b(0x7fc0)), "nan decode");
        check(to_h(0.0f) == 0x0000 && to_h(-0.0f) == 0x8000, "half signed zero");
        check(to_b(0.0f) == 0x0000 && to_b(-0.0f) == 0x8000, "bf16 signed zero");
        ...
        check(of_b(0x7f7f) == 3.3895313892515355e+38f, "bf16 max normal is 0x7f7f");
        check(to_b(3.4028235e38f) == 0x7f80, "FLT_MAX overflows bf16 -> +inf");

        // arithmetic goes through float, and half OP half stays half
        static_assert(std::is_same<decltype(half(1) + half(1)), half>::value, "half + half -> half");
        check(static_cast<float>(half(1.5f) * half(2.0f)) == 3.0f, "half multiply");
        half acc(0.0f); acc += half(0.5f); acc += half(0.25f);
        check(static_cast<float>(acc) == 0.75f, "half operator+=");
        check(half(2.0f) > half(1.0f) && !(half(2.0f) < half(1.0f)), "half compare");
        ...
        static_assert(std::is_same<decltype(half(1) * 2.0f),  float>::value,  "half * float  -> float");
        static_assert(std::is_same<decltype(2.0 * half(1)),   double>::value, "double * half -> double");
        static_assert(std::is_same<decltype(half(1) - 1),     int>::value,    "half - int    -> int");
        ...
        // accumulator sticks. compute_type<half> == float is why we don't.
        {
            ...
            half narrow(0.0f);
            for (int i = 0; i < 2049; ++i) { wide += 1.0f; narrow += half(1.0f); }
            check(wide == 2049.0f, "float accumulator reaches 2049");
            check(static_cast<float>(narrow) == 2048.0f,
