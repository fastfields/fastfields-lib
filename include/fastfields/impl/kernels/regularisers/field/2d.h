#ifndef FF_REGULARISERS_FIELD_2D
#define FF_REGULARISERS_FIELD_2D
...
struct Kernels<Config<two, _C, T...>>
{
    ...
    static constexpr offset_t D   = static_cast<offset_t>(two);
...
#       pragma unroll
    ...
    )
    {
        reduce_t vx = voxel_size[0], vy = voxel_size[1];
        vx = 1./(vx*vx); vy = 1./(vy*vy);
        for (offset_t c=0; c<(C < 0 ? nc : C); ++c, kernel+=6)
        {
    ...
    )
    {
        ...
        // and `kernel[2]` mix a bending *and* a membrane contribution. Build
        // the table directly instead, applying 0.25 to the bending part and
        // 0.5 to the membrane part. `kernel[0]` (absolute) is left unweighted.
        reduce_t vx = voxel_size[0], vy = voxel_size[1];
        vx = 1. / (vx * vx);
        vy = 1. / (vy * vy);
        for (offset_t c = 0; c < (C < 0 ? nc : C); ++c, kernel += 6) {
...
#endif // FF_REGULARISERS_FIELD_2D
